      ...
      <div className="grid md:grid-cols-2 gap-8 md:gap-12">
        ...
        <div>
          ...
          <div className="mt-8">
            ...
            <div className="flex gap-2 flex-wrap">
              {["S", "M", "L", "XL", "XXL"].map((s) => (
                <button
                  key={s}
                  className="border-2 border-neutral-300 hover:border-red-600 w-12 h-12 rounded-lg font-bold transition"
                >
                  {s}
                </button>
              ))}
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            <a
              href={`https://wa.me/56900000000?text=${encodeURIComponent(`Hola, me interesa la camiseta ${product.name}`)}`}
              target="_blank"
              rel="noreferrer"
              className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-4 px-6 rounded-full text-center transition"
            >
              Comprar por WhatsApp
            </a>
            <button className="flex-1 border-2 border-neutral-900 hover:bg-neutral-900 hover:text-white font-bold py-4 px-6 rounded-full transition">
              Agregar al carrito
            </button>
          </div>
